<template>
  <div class="event-card-container course-card-container">
    ...
    <el-card class="event-item" shadow="hover" @click.stop="$emit('create-event')">
      <h3>Create Event</h3>
      <el-icon :size="24" style="margin-top: 10px;">
        <DocumentAdd />
      </el-icon>
    </el-card>
    <el-card v-for="event in attendanceEvents" :key="event.id" class="event-item" shadow="always"
      ...
      <div style="">
        ...
        <el-icon :size="18" @click="showAttendanceMap(event)">
          <MapLocation />
        </el-icon>
    ...
    <el-dialog v-model="attendanceMapVisible" :title="selectedEvent.name" :width="dialogWidth">
      <AttendanceMap v-if="attendanceMapVisible" @closeDialog="closeDialog" :enrollments="enrollments"
        :eventAttendances="eventAttendances" :event="selectedEvent"></AttendanceMap>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    closeDialog() {
      this.attendanceMapVisible = false;
    },
    setDialogWidth() {
